...
void FormatConversionDialog::performConversion()
{
    ...
    for (int i = 0; i < m_selectedFiles.size(); i++) {
        ...
        // 构建新文件名
        QString newPath = info.path() + "/" + info.completeBaseName() + "." + targetFormat;
        ...
        if (QFile::exists(newPath)) {
            ...
            while (QFile::exists(newPath)) {
                newPath = info.path() + "/" + info.completeBaseName() +
                         QString("_%1.").arg(counter) + targetFormat;
                counter++;
            }
        }

        // 执行重命名（实际应用中这里应该是真正的格式转换）
    ...
    QMessageBox::information(this, "转换完成",
        QString("成功转换 %1 个文件\n跳过 %2 个不匹配的文件")
        .arg(successCount).arg(skippedCount));
